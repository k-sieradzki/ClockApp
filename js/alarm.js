...
const valueSelected = document.querySelector('.alarmMenu .ringtone .selected')
...
const playAlarm = () => {
	...
	const funTest2 = () =>{
		switch (parseInt(alarmex[i].alarmid)) {
			case 1:
			value = 1;
			audio = new Audio('../audio/BeepAlarm.mp3');
			...
			case 2:
			value = 2;
			audio = new Audio('../audio/CorporateMusic.mp3');
			...
			case 3:
			value = 3;
			audio = new Audio('../audio/FolkPop.mp3');
			...
			case 4:
			value = 4;
			audio = new Audio('../audio/HappyBouzouki.mp3');
			...
			case 5:
			value = 5;
			audio = new Audio('../audio/WonderfulGuitar.mp3');
	...
	for(i=0; i<alarmex.length; i++){
		...
		if(alarmex[i].timex === date1.getTime() && clasCheck.classList.contains('showC')){
			funTest2()
			alarmex[i].timex = ''
			labelSpan = document.querySelector(`.alarm${i} .alarmBox label span`)
			labelSpan.click()
		}
	}
}
...
const createNewAlarm = () =>{
	if(setMinute.value == '' || setHour.value == ''){
		return;
	}else{
